...
async function checkUserName(userName) {
  try {
    ...
    const response = await fetch("../../BLL/AccountBLL.php", {
      ...
      body:
        "function=" +
        encodeURIComponent("checkUserName") +
        "&userName=" +
...
async function signUp(
  ...
  try {
    ...
    const response = await fetch("../../BLL/AccountBLL.php", {
      ...
      body:
        "function=" +
        encodeURIComponent("add_Account") +
        "&userName=" +
        encodeURIComponent(userName) +
        "&passWord=" +
        encodeURIComponent(passWord) +
        "&dateCreate=" +
        encodeURIComponent(dateCreate) +
        "&accountStatus=" +
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  form.addEventListener("submit", async function (event) {
    ...
    // Regular expressions
    var nameRegex =
      /^[a-zA-Z\sÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚĂĐĨŨƠàáâãèéêìíòóôõùúăđĩũơƯĂẠẢẤẦẨẪẬẮẰẲẴẶẸẺẼỀỀỂỄỆỈỊỌỎỐỒỔỖỘỚỜỞỠỢỤỦỨỪễệỉịọỏốồổỗộớờởỡợụủứừỬỮỰỲỴÝỶỸửữựỳỵỷỹ]+$/u;
